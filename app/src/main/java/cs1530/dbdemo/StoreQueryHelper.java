...
public class StoreQueryHelper {
    ...
    public StoreQueryHelper(Connection conn) {
        ...
        try (Statement st = databaseConnection.createStatement()) {
            ...
            st.executeUpdate(
                    "INSERT INTO STORE VALUES (1, 'Test Store', 'sitting', 'Forbes', 'Pittsburgh', 'Pennsylvania');");
            st.executeUpdate(
                    "INSERT INTO STORE VALUES (2, 'Test Store2', 'sitting', 'Fifth', 'Pittsburgh', 'Pennsylvania');");

            // Or execute multiple updates within a single statement
            st.executeUpdate(
                    "INSERT INTO STORE VALUES (3, 'Test Store 3', 'drive-through', 'Lincoln', 'Seattle', 'Washington'),"
                            +
    ...
    protected List<RowInterface> findAll() {
        ...
        try (Statement st = databaseConnection.createStatement()) {
            st.setQueryTimeout(QUERY_TIMEOUT); // set timeout to 30 seconds
            // TODO: Replace the Null assignment below with the SQL query to retrieve all
            // stores
    ...
    protected List<RowInterface> findByStoreNumber(int storeNumber) {
        List<RowInterface> storeList = new ArrayList<>();
        // TODO: Fill-In the SQL statement below to filter for rows with a given
        // storeNumber
    ...
    protected List<RowInterface> findByCityAndState(String city, String state) {
        List<RowInterface> storeList = new ArrayList<>();
        // TODO: Fill-In the SQL statement below to filter for rows with a given city
        // and state
        try (PreparedStatement st = databaseConnection
                .prepareStatement("SELECT * FROM STORE WHERE CITY = ? AND STATE = ?")) {
            st.setQueryTimeout(QUERY_TIMEOUT);
     ...
     *                      such as trying to access a column that is not part of
     *                      the ResultSet
     */
